...
int mpireorderinglib::kd_Tree_Reorderer::perform_reordering(MPI_Comm old_comm,
										   int ndims,
										   const int *dims,
										   const int *periods,
										   const int *stencil,
	...
	return MPIX_skewed_kdtree_cart_create(old_comm, dims, ndims, stencil, n_neighbors, periods, cart_comm);
}
